...
class Component {
  ...
  public function selectedMethods() {
    ...
    return \Drupal::entityManager()->getStorage('payment_method');
  }
  ...
  protected function getMethods($context) {
    ...
    \Drupal::moduleHandler()->alter('webform_paymethod_select_method_list', $context, $methods_copy, $methods);
    return $methods;
  }
  ...
  public function render(&$element, &$form, &$form_state) {
    ...
    foreach($methods as $pmid => $payment_method) {
      $pmid_options[$pmid] = \Drupal\Component\Utility\SafeMarkup::checkPlain(t($payment_method->title_generic));
    }
    ...
    $element += array(
      ...
      '#id' => \Drupal\Component\Utility\Html::getId('paymethod-select-wrapper'),
